...
from ..errors import ReservedId, RevisionAlreadyPresent, RevisionNotPresent
...
## Tests SuicideDelete, CannedDelete, CannedReplacement, BadWeave, BadInsert,
## and InsertNested were ported to Rust unit tests in
## crates/bazaar/src/weave.rs (see `canned_delete_round_trip`,
## `canned_replacement_round_trip`, `insert_nested_round_trip`, and the
## adjacent invalid-shape tests). They poked at `Weave._weave`, `_parents`,
## and `_sha1s` directly, which the Rust-backed `Weave` exposes read-only.
...
## Tests IncludeVersions and DivergedIncludes were ported to Rust unit
## tests `include_versions_round_trip` and `diverged_includes_round_trip`
## in crates/bazaar/src/weave.rs.
