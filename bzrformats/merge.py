...
from ._bzr_rs.plan_merge import _PlanLCAMerge  # noqa: F401
# `_PlanLCAMerge(a_rev, b_rev, vf, key_prefix, graph)`: the constructor
# computes the merge plan eagerly via the pure-Rust port in
# `crates/bazaar/src/plan_merge.rs`. `plan_merge()` then yields the
# cached `(tag, line)` tuples; `_subtract_plans` is the same Rust helper
# `_PlanMerge` reuses as a classmethod.
